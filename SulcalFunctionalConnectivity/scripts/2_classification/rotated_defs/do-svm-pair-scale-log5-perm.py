...
data = np.zeros((n_subs,42,42))
for s in np.arange(n_subs):
    fc_file = '%s/rotated/sub-%s/ses-%s/seed-fc_labels_hybrid_perm%d.csv'%(datadir,subs[s],subs[s][-1],perm)
    data[s,:] = np.array(pd.read_csv(fc_file))

outdir = '%s/perm_results'%os.getcwd()
if not os.path.exists(outdir):
    os.makedirs(outdir)

# the rest as for the main analysis classification
n_subs = shape(data)[0]
...
savez('%s/sulcal-classification-nosa-pair-scaled-%d.npz'%(outdir,perm),conf_mat=total_conf_mat,coefs=total_coefs,total_acc=total_acc,total_p=total_p)
