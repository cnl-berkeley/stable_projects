...
def filterBasedOnLabels(df,tag,pipe):
...
def saveCohortData(data_all,sub_ids,savedir):
    ...
    # save used subjects
    savetxt('%s/namelist.csv'%savedir,sub_ids,delimiter=',',header='',fmt='%s')
    ...
    for i in arange(len(sub_ids)):
        ...
        savetxt('%s/fc-labels-both.csv'%savedir2,data,delimiter=',',header='',fmt='%.04f')
        all_vals[i,:,:] = array(data)

    # save mean residual data
    mean_data = squeeze(nanmean(all_vals,0))
    savetxt('%s/raw/mean_data-both.csv'%savedir,mean_data,delimiter=',',header='',fmt='%.04f')

    # save group variance residual data
    var_data = squeeze(nanvar(all_vals,0))
    savetxt('%s/raw/var_data-both.csv'%savedir,var_data,delimiter=',',header='',fmt='%.04f')

    # save data deviations from residual mean
    if not os.path.exists('%s/dev_from_mean'%savedir):
        os.makedirs('%s/dev_from_mean'%savedir)
    for i in arange(len(sub_ids)):
        data = array(all_vals[i])
        data = subtract(data,mean_data)
...
if __name__ == '__main__':
    ...
    savedir = '%s/%s/%s'%(os.getcwd(),pipe,pipeline)
    if not os.path.exists(savedir):
        os.makedirs(savedir)

    for tag in ['','-sareg']:
        # filter out subjects without connectivity values on required labels
        data,sub_fds,sub_ids = filterBasedOnLabels(df,tag,pipe)
