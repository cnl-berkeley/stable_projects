...
def procSub(postproc_dir,fmriprep_derivatives_dir,sub,ses,task,subtasks,labelname,runs,rot_file,hemi,tmpdir,outdir):
    ...
    if hemi == 'rh':
        highres_func = '%s/sub-%s/ses-%s/hemi-lh/lev1/run-%s/res4d.func.gii'%(postproc_dir,sub,ses,runs[0])
        ts,affine = getRunData(highres_func)
        for i in np.arange(len(label_inds_all)):
            tmp = np.array(label_inds_all[i]) - ts.shape[1]
            label_inds_all[i] = list(tmp)
    ...
    procLabels(postproc_dir,sub,ses,hemi,task,subtasks,labelname,label_inds_all,savedir,highres_funcs)


def procLabels(postproc_dir,sub,ses,hemi,task,subtasks,labelname,label_inds_all,savedir,highres_funcs):
    ...
    for subtask in subtasks:
        ...
        savename = 'rotated-%s_%s_nw_%s'%(hemi,labelname,subtask[0])
        procTask(postproc_dir,sub,ses,hemi,task,runs,subtask,labelname,label_inds_all,annots,annot_inds,savedir,savename,highres_funcs)
...
if __name__ == '__main__':
    ...
    label = sys.argv[3]
    hemi = sys.argv[4]
    print(sub,ses,runs,label)

    rot_dir = '/home/weiner/shakki/scripts-github/analysis/network_correlations/generate_rotated_labels'
    labelname = '%s.%s'%(hemi,label)
    rot_file = '%s/sub-%s/rotated_labels/%s_rotations.csv'%(rot_dir,sub,labelname)

    procSub(postproc_dir,fmriprep_derivatives_dir,sub,ses,task,subtasks,labelname,runs,rot_file,hemi,tmpdir,outdir)
