...
### ONLY THIS LINE NEEDS MODIFICATION ###
### UPDATE WITH DESIRED LABELS ###
middle_frontal_label_names = np.array(['pimfs_d', 'pimfs_v'])
#middle_frontal_label_names = np.array(['central', 'painfs', 'pmfs_i', 'pmfs_a', 'pmfs_p''iprs','sprs','ifs', 'sfs_a', 'infs_v', 'infs_h', 
...
for i, middle_frontal_label in enumerate(middle_frontal_label_names):
    ...
    for sub in subjects:
        try:
            ...
            label_path_lh = prediction_fsaverage_space_labels + '/{}.lh.{}.label'.format(sub, middle_frontal_label)
    ...
    for sub in subjects:
        try:
            label_path_rh = prediction_fsaverage_space_labels + '/{}.rh.{}.label'.format(sub ,middle_frontal_label)
...
vertices_lh, RAS_lh = read_label('/home/weiner/Nora_PFCSulci/subjects/fsaverage/label/lh.cortex.label')
...
for vtx in vertices_lh:
	...
	# for unthresholded --> if (np.max(stat_with_vtx) > 0):
    if (labels_with_vtx.shape[0] > 0):

        if (np.max(stat_with_vtx) > 0):
...
# for unthreshold --> lh.{}_PROB_MPM_unthresh_{}.label

for i, middle_frontal_label in enumerate(middle_frontal_label_names):

    MPM_path = fsaverage_space_labels + '/prob_maps/{}/lh.{}_PROB_MPM_unthresh_{}.label'.format(left_out_sub, project_id, middle_frontal_label)
    try:
...
vertices_rh, RAS_rh = read_label('/home/weiner/Nora_PFCSulci/subjects/fsaverage/label/rh.cortex.label')
...
for vtx in vertices_rh:
    ...
    for label_prob, vertices_prob in prob_maps_vertices.items():
     ...
     # if vertex was in at least one prob map, get the max value and add to MPM file
        # also required to have a probability of 0.33 or higher
	# unthresholded --> if (np.max(stat_with_vtx) > 0):
    if (labels_with_vtx.shape[0] > 0):
...
# save out each entry MPM as a separate label file in Freesurfer
# for unthresholded rh.{}_PROB_MPM_unthresh_{}.label

for i, middle_frontal_label in enumerate(middle_frontal_label_names):

    MPM_path = fsaverage_space_labels + '/prob_maps/{}/rh.{}_PROB_MPM_unthresh_{}.label'.format(left_out_sub, project_id, middle_frontal_label)
    try:
        # make probabilistic label array for albel file
        prob_array = np.zeros(shape=(MPM_vertices[middle_frontal_label].shape[0],5),dtype=float)
        prob_array[:,0] = MPM_vertices[middle_frontal_label]
